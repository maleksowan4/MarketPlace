...
const server = http.createServer((req, res) => {
  ...
  let targetServiceUrl = SERVICES.auth;

  // Static Uploads Routing
  if (
    path.startsWith('/api/uploads/user') ||
    path.startsWith('/uploads/user')
  ) {
    targetServiceUrl = SERVICES.user;
  } else if (
    path.startsWith('/api/uploads/product') ||
    path.startsWith('/uploads/product')
  ) {
  ...
  // Uploads path rewriting
  if (
    path.startsWith('/api/uploads/user') ||
    path.startsWith('/uploads/user')
  ) {
    rewrittenUrl = path.replace(
  ...
  const shopProductsMatch = path.match(
    /^\/api\/shops\/([0-9]+)\/products$/
  );
  ...
  console.log(
    `[Gateway] Proxying ${method} ${path} -> ${targetUrl.origin}${rewrittenUrl}`
  );

  // Forward the original request headers
  // but replace Host with the target service host.
  const proxyHeaders = {
    ...req.headers,
    host: targetUrl.host,
  };

  // Remove Azure/proxy forwarding headers.
  // These headers describe the original public request and
  // should not be forwarded to internal Container Apps.
  delete proxyHeaders['x-forwarded-host'];
  delete proxyHeaders['x-forwarded-proto'];
  delete proxyHeaders['x-forwarded-port'];
  delete proxyHeaders['forwarded'];
  ...
  // Use HTTPS for HTTPS service URLs.
  // Use HTTP for HTTP service URLs.
  const transport =
  ...
  proxyReq.on('error', (err) => {
    console.error(
      `[Gateway Error] Failed to proxy request to ${targetUrl.origin}: ${err.message}`
    );
